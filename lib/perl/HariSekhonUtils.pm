...
BEGIN {
    # needs to be before die_sub(), otherwise could get 'Use of uninitialized value $KhulnaSoftUtils::ERRORS{"CRITICAL"} in exit' and exit with blank / 0 incorrect error code on early stage failures such as 'This Perl not built to support threads'
    #
...
our $nagios_plugins_support_msg = "Please try latest version from https://github.com/KhulnaSoft/Nagios-Plugins, re-run on command line with -vvv and if problem persists paste full output from -vvv mode in to a ticket requesting a fix/update at https://github.com/KhulnaSoft/Nagios-Plugins/issues/new";
...
sub remove_timeout(){
    delete $KhulnaSoftUtils::default_options{"t|timeout=i"};
}
...
sub strip_ansi_escape_codes ($) {
    ...
    # ported from https://github.com/KhulnaSoft/pylib
...
sub usage (;@) {
    ...
    if(not @_ and $main::DESCRIPTION){
        print STDERR "KhulnaSoft Ltd - https://github.com/khulnasoft";
        if($github_repo){
...
sub version_string () {
    ...
    $version_str .= "KhulnaSoft Ltd Utils version $KhulnaSoftUtils::VERSION";
    return $version_str;
}
