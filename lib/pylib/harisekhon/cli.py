...
import khulnasoft
from khulnasoft.utils import log, getenvs2, isBlankOrNone, isInt, isHost, isPort, isStr, isList, validate_int
from khulnasoft.utils import CodingError, InvalidOptionException, ERRORS, qquit  # , die
from khulnasoft.utils import get_topfile, get_file_docstring, get_file_github_repo, get_file_version, plural
from khulnasoft.utils import CriticalError, WarningError, UnknownError
...
class CLI(object):
    ...
    def __init__(self):
        ...
        self._utils_version = khulnasoft.utils.__version__
        ...
        self.usagemsg = 'KhulnaSoft Ltd{sep}{github_repo}\n\n{prog}\n{docstring}\n'.format(
            sep=' - ' if self._github_repo else '',
            github_repo=self._github_repo,
            prog=self._prog,
            docstring=self._docstring)
        self.usagemsg_short = 'KhulnaSoft Ltd%(_github_repo)s\n\n' % self.__dict__
    ...
    def main(self):
        ...
        try:
            ...
            log.info('KhulnaSoft Ltd %s', self.version)
...
#                raise
    ...
    def run(self):  # pragma: no cover
        raise CodingError('running KhulnaSoft.CLI().run() - this should be abstract and non-runnable!'
