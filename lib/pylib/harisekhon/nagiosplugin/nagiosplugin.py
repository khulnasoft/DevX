...
from khulnasoft.utils import ERRORS, qquit, CodingError, log, isStr, log_option, support_msg
from khulnasoft.utils import WarningError, CriticalError, UnknownError
...
class NagiosPlugin(CLI):
    """
    KhulnaSoft.NagiosPlugin class for rapid development of Nagios Plugins in Python
    """
    ...
    def status(self, status):
        if status not in ERRORS:
            raise CodingError("invalid status '{0}' passed to khulnasoft.NagiosPlugin.status()".format(status))
