...
# https://github.com/KhulnaSoft/Jenkins/blob/master/vars/download.sh

# Downloads and replaces existing groovy files in the local directory with the latest function/pipeline files from KhulnaSoft/Jenkins if they exist upstream
#
...
#       cd vars/
#
#       curl -f https://raw.githubusercontent.com/KhulnaSoft/Jenkins/master/vars/download.sh > download.sh && chmod +x download.sh
#
# Download or update any number of functions you want, given as their groovy filenames as you see in the KhulnaSoft/Jenkins repo::
#
#       ./download.sh argoDeploy.groovy
#
# Or run without any args to search for every *.groovy file in the local directory under vars/ and, if available in KhulnaSoft/Jenkins repo, overwrite it with the latest version from GitHub
#
#       ./download.sh
#
...
url="https://raw.githubusercontent.com/KhulnaSoft/Jenkins/master/vars"
checkout=~/github/jenkins

if git remote -v | grep -q '^origin[[:space:]].*KhulnaSoft/Jenkins'; then
...
if [ -d "$checkout" ]; then
    ...
    if ! git remote -v 2>/dev/null | grep -qi 'origin.*khulnasoft'; then
        checkout=""
    fi
    popd >/dev/null
fi
