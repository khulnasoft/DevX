...
$ua->agent("KhulnaSoft Ltd $progname version $main::VERSION");
    ...
    if($all_metrics){
        ...
        foreach my $metric (@stats){
            ...
            if(isHash($json->{$metric})){
                defined($json->{$metric}{"last"}) or quit "UNKNOWN", "unrecognized metric/format detected for '$metric', ask author KhulnaSoft Ltd for an update";
