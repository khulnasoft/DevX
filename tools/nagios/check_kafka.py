...
try:
    ...
    from khulnasoft.utils import log, log_option, ERRORS, CriticalError, UnknownError
    from khulnasoft.utils import validate_hostport, validate_int, get_topfile, random_alnum, validate_chars, isSet
    from khulnasoft import PubSubNagiosPlugin
except ImportError:
    print(traceback.format_exc(), end='')
    sys.exit(4)
...
class CheckKafka(PubSubNagiosPlugin):
    ...
    def __init__(self):
        ...
        self.consumer = None
        self.topic = None
        self.client_id = 'KhulnaSoft Ltd {prog} {version}'.format(prog=os.path.basename(get_topfile()),
                                                               version=__version__)
