...
if($results{"domain"}){
    ...
    if(%domain_mismatches){
        warning;
        $msg = sprintf("mismatching domain found multiple (%d) times in registrar output: %s - raise a ticket on github for a fix https://github.com/KhulnaSoft/Nagios-Plugins/issues. %s", scalar keys %domain_mismatches,  join(" vs ", sort keys %domain_mismatches), $msg);
    }
}
