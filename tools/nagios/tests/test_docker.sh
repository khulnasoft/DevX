...
export DOCKER_IMAGE="khulnasoft/nagios-plugins"
export DOCKER_IMAGE_TAGS="latest centos debian ubuntu alpine"
export DOCKER_IMAGES=(khulnasoft/tools khulnasoft/pytools khulnasoft/nagios-plugins)
if is_CI; then
    # want splitting
...
if is_docker_available; then
    ...
    if is_CI; then
        ...
        trap '
            docker_rmi_grep khulnasoft/nagios-plugins || :
            docker_rmi_grep khulnasoft/tools || :
            docker_rmi_grep khulnasoft/pytools || :
            docker_rmi_grep khulnasoft/perl-tools || :
            docker_rmi_grep khulnasoft/bash-tools || :
        ' $TRAP_SIGNALS
    fi
