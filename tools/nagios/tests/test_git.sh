...
if [ -z "$(git config user.name)" ]; then
    echo "setting git user.name KhulnaSoft Ltd in local repo to allow commit"
    git config $global user.name "KhulnaSoft Ltd"
fi
if [ -z "$(git config user.email)" ]; then
    echo "setting git user.email khulnasoft@gmail.com in local repo to allow commit"
    git config $global user.email "khulnasoft@gmail.com"
fi
