...
if [ "${HBASE_IMAGE:-}" = hbase ]; then
    echo "checking khulnasoft/hbase image, skipping docker exec checks..."
    export HBASE_IMAGE
    export DOCKER_SKIP_EXEC=1  # don't run local tests which requires khulnasoft/hbase-dev containing pre-built dependencies
fi
