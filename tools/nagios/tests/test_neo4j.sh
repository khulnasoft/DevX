...
test_neo4j_main(){
    ...
    if [ -z "${NOSETUP:-}" ]; then
        ...
        if [ "${version:0:3}" = "2.3" ] || [ "${version:0:3}" = "3.0" ]; then
            ...
            docker-compose exec "$DOCKER_SERVICE" /var/lib/neo4j/bin/neo4j-shell -host localhost -c 'CREATE (p:Person { name: "KhulnaSoft Ltd" });'
        else
            # connects to 7687
            ...
            docker exec -i $auth_env "$DOCKER_CONTAINER" /var/lib/neo4j/bin/cypher-shell <<< 'CREATE (p:Person { name: "KhulnaSoft Ltd" });'
        fi
        hr
    fi
    if [ "${NOTESTS:-}" ]; then
