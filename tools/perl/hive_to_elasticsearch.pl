...
use KhulnaSoftUtils qw/:DEFAULT :regex :time/;
use KhulnaSoft::Elasticsearch;
use Cwd 'abs_path';
    ...
    if(@partitions_found){
        ...
        foreach my $partition (@partitions_found){
            ...
            if($partition =~ /^([$valid_partition_key_chars]+=[$valid_partition_value_chars]+)$/){
                ...
                quit "UNKNOWN", "invalid partition '$partition' detected in Hive table when attempting to iterate and index all partitions. Re-run with -vvv and paste in to a ticket at the following URL for a fix/update: https://github.com/KhulnaSoft/DevOps-Perl-tools/issues";
            }
            indexToES($index, $partition);
        }
    } else {
