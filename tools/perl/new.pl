...
BEGIN {
    ...
    # wiped out by KhulnaSoftUtils for taint safety, restore for $EDITOR as vim relies on it for lots of special customizations in ~/.vimrc in DevOps-Bash-tools repo
    $main::ORIGINAL_PATH = $ENV{'PATH'};
}
use KhulnaSoftUtils qw/:DEFAULT :regex/;
use Carp;
...
sub load_vars($$$){
    ...
    if(!$vars{"URL"}){
        ...
        $vars{"URL"}  = "https://github.com/KhulnaSoft";
        if($ENV{"PWD"} =~ /\/work/){
            # don't add suffix
    ...
    $vars{"LINKEDIN"} = "https://www.linkedin.com/company/khulnasoft";
